...
Repo kökündeki `Dockerfile` uygulamayı .NET 10 ile yayınlar. `render.yaml`, Render üzerinde
Frankfurt bölgesinde ücretsiz bir Docker web servisi tanımlar. Blueprint kurulurken
`ConnectionStrings__HotelDatabase` değeri Supabase Session pooler bağlantı dizesi olarak girilir.
`main` dalının GitHub kontrolleri geçince yeni sürüm otomatik yayınlanır. `/health` yolu
veritabanına sorgu göndermeden uygulamanın ayakta olduğunu bildirir.
